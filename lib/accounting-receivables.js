...
export async function customerLedgerHistory(db, { from = null, to = null, status = 'all', search = null, limit = 200 } = {}) {
  ...
  else if (status === 'paid') where += " AND cl.entry_type IN ('credit_payment', 'adjustment')";
  if (search) { where += ' AND c.name LIKE ?'; params.push(`%${search}%`); }
  params.push(Math.min(500, Math.max(1, Number(limit) || 200)));

  const rows = await db.all(
    `SELECT cl.id, cl.created_at AS date, cl.entry_type, cl.debit, cl.credit, cl.note,
  ...
  const STATUS_LABEL = { credit_sale: 'charged', credit_payment: 'paid', adjustment: 'cleared' };
  return (rows || []).map((r) => ({
    id: r.id,
    date: r.date,
    ...
    order_id: r.order_id,
    status: STATUS_LABEL[r.entry_type] || r.entry_type,
    debit: round2(r.debit),
    credit: round2(r.credit),
    note: r.note,
  }));
