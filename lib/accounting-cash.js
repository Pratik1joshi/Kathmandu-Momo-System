...
export async function openSession(db, { drawer_id, opening_amount = 0, opened_by = null, note = null }) {
  ...
  return db.transaction(async (tx) => {
    ...
    try {
      await tx.run(
        `INSERT INTO drawer_sessions (drawer_id, status, opening_amount, opened_by, note) VALUES (?, 'open', ?, ?, ?)`,
        [drawerId, opening, opened_by, note]
      );
...
export async function settlePayments(db, { method, gross_amount, fee_amount = 0, bank_account_id, reference, note, settled_by, external_ref = null, allow_over = false }) {
  ...
  return db.transaction(async (tx) => {
    ...
    await tx.run(
      `INSERT INTO payment_settlements (method, gross_amount, fee_amount, net_amount, bank_account_id, reference, note, settled_by, journal_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [method, gross, fee, net, bank_account_id, reference || null, note || null, settled_by, journalId]
    );
