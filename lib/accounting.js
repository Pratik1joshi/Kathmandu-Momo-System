 ...
 */
export async function postJournal(db, { entry_date, memo, source_type = null, source_id = null, external_ref = null, created_by = null, lines }) {
  ...
  const res = await db.run(
    `INSERT INTO journal_entries (entry_date, memo, source_type, source_id, external_ref, created_by)
     VALUES (COALESCE(?, CURRENT_DATE), ?, ?, ?, ?, ?)`,
    [entry_date || null, memo || null, source_type, source_id, external_ref, created_by]
  );
  const journalId =
    res?.lastInsertRowid ??
    (await db.get(
  ...
  for (const l of resolved) {
    await db.run(
      `INSERT INTO journal_lines (journal_id, account_id, debit, credit, memo, drawer_id, bank_account_id, supplier_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [journalId, l.account_id, round2(l.debit), round2(l.credit), l.memo || null, l.drawer_id || null, l.bank_account_id || null, l.supplier_id || null]
    );
  }
  return journalId;
}
 ...
 */
export async function postSaleJournal(db, { bill_id, bill_number, entry_date, parts, created_by }) {
  const clean = (parts || []).map((p) => ({ method: p.method, amount: round2(p.amount) })).filter((p) => p.amount > 0);
  if (!clean.length) return null;
  const drawerId = await currentDrawerId(db);
  const lines = clean.map((p) => {
    const code = paymentAccountCode(p.method);
    return { code, debit: p.amount, credit: 0, drawer_id: code === '1010' ? drawerId : null, memo: `${p.method} payment` };
  });
  const total = round2(clean.reduce((s, p) => s + p.amount, 0));
  lines.push({ code: '4010', debit: 0, credit: total, memo: 'Sales revenue' });
