...
export async function buildOrderOperationsAnalytics(db, range, businessDayId = null) {
  ...
  const bills = billsRaw.map((bill) => {
    ...
    return {
      ...
      gross: round2(n(bill.subtotal) + n(bill.discount_amount)), discount: round2(bill.discount_amount),
      discountPercent: n(bill.subtotal) + n(bill.discount_amount) ? round2(n(bill.discount_amount) / (n(bill.subtotal) + n(bill.discount_amount)) * 100) : 0,
