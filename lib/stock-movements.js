...
import { buildSearch, paginateQuery, resolveOrderBy } from './paginate.js';
...
export async function logStockMovement(db, { inventory_item_id, change_type, quantity_changed, performed_by, reason, reference_id }) {
  if (!inventory_item_id || !change_type || !Number.isFinite(Number(quantity_changed))) return;
  await db.run(
    `INSERT INTO stock_movements (inventory_item_id, change_type, quantity_changed, performed_by, reason, reference_id)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [inventory_item_id, change_type, Number(quantity_changed), performed_by || null, reason || null, reference_id ? String(reference_id) : null]
  );
}
