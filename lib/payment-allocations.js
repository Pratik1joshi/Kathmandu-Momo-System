...
export function validateAllocations(rawAllocations, expectedAmount, {
  ...
  for (const row of allocations) {
    ...
    if (row.method === 'credit') {
      ...
      if (actorRole !== 'admin') fail('Administrator permission is required for credit.', 403);
      ...
      const limit = toCents(customer.credit_limit);
      const current = toCents(customer.current_credit);
      if (limit <= 0 || current + row.cents > limit) fail('This credit allocation exceeds the customer credit limit.');
    }
  }
  return allocations;
}
