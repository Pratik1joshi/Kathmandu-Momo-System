...
export async function getPublicMenuCategories() {
  ...
  try {
    const rows = await menuRepo.db.all(`
      SELECT v.menu_item_id, v.variant_name, v.price_modifier, v.is_default, mi.base_price
      FROM menu_item_variants v
      JOIN menu_items mi ON mi.id = v.menu_item_id
      WHERE mi.is_available = 1
      ORDER BY v.is_default DESC, v.price_modifier
    `)
    for (const r of rows || []) {
      ...
      variantsByItem.get(key).push({
        name: r.variant_name,
        price: Number(r.base_price) + Number(r.price_modifier || 0),
      })
    }
  } catch (e) {
