...
export async function autoCloseStaleBusinessDay(db) {
  ...
  return db.transaction(async (tx) => {
    ...
    // The store session was already closed and physically counted (e.g. via
    // the "Close Store Session" button) but nobody opened the next business
    // day to finalize it — finalizeBusinessDay honors that real count. Only
    // guess counted=expected when the session itself was left open (truly
    // forgotten), so midnight never overwrites a cashier's real count.
    if (!active) {
      await finalizeBusinessDay(tx, day, null, {
        reason: 'Automatically finalized after Nepal midnight; store session was already closed.',
      });
      return { closed: true, business_day_id: day.id };
    }
