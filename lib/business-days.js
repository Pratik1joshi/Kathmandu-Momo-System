...
const addDays = (dateValue, days) => {
  const date = new Date(`${String(dateValue).slice(0, 10)}T12:00:00+05:45`);
...
export function isStaleBusinessDay(day) {
  return !!day && day.status === 'open' && String(day.business_date).slice(0, 10) < getNepaliDateString();
}
...
export async function businessDayContext(db) {
  ...
  return {
    ...
    activeSession, latestSession, previous: previous || null,
...
export async function openBusinessDay(db, data, actor) {
  ...
  const businessDate = String(data.business_date || getNepaliDateString()).slice(0, 10);
  const openingCash = round2(data.opening_cash);
  ...
  return db.transaction(async (tx) => {
    ...
    if (existing) {
      ...
      if (action === 'start_next') {
        ...
        if (businessDate <= String(existing.business_date).slice(0, 10)) {
...
export async function businessDaySummary(db, dayOrId) {
  ...
  const [sales, channels, paymentRows, corrections, expenses, orders, kots, itemTotals, topItems, tables, cashRows, onlineRows, blockers] = await Promise.all([
    db.get(`SELECT COUNT(*) AS completed_bills, COALESCE(SUM(subtotal),0) AS gross_sales,
    ...
    db.all(`SELECT LOWER(COALESCE(o.order_type,'other')) AS channel, COALESCE(SUM(b.grand_total),0) AS amount, COUNT(*) AS bills
      FROM bills b JOIN orders o ON o.id=b.order_id WHERE b.business_day_id=? AND ${settled.replaceAll('status', 'b.status')}
  ...
  return {
    ...
    sales: {
      gross_sales: round2(sales?.gross_sales), discounts: round2(sales?.discounts), net_sales: round2(sales?.net_sales),
      tax: round2(sales?.tax), service_charge: round2(sales?.service_charge), billed_total: round2(sales?.billed_total),
      refunds: round2(corrections?.refunds), voided_bill_value: round2(corrections?.voided_value),
      completed_bills: Number(sales?.completed_bills || 0), average_bill: round2(sales?.average_bill), items_sold: itemsSold,
      channels: (channels || []).map((row) => ({ ...row, amount: round2(row.amount), bills: Number(row.bills || 0) })),
    },
