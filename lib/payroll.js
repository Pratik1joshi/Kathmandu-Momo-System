...
export async function recordAdvance(db, data, givenBy = null) {
  ...
  return db.transaction(async (tx) => {
    const result = await tx.run(
      ...
      [employeeId, amount, data.advanced_on || null, data.method || 'cash', data.note || null, givenBy, businessDayId]
    );
  ...
  return db.transaction(async (tx) => {
    await tx.run(
      `INSERT INTO salary_payments (employee_id, amount, gross_amount, advance_deduction, period_label, paid_on, method, note, paid_by, business_day_id)
       VALUES (?, ?, ?, ?, ?, COALESCE(?, CURRENT_DATE), ?, ?, ?, ?)`,
      [data.employee_id, amount, grossAmount, advanceDeduction, data.period_label || null, data.paid_on || null, data.method || 'cash', data.note || null, paidBy, businessDayId]
    );
