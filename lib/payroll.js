...
export async function recordPayment(db, data, paidBy = null) {
  ...
  return db.transaction(async (tx) => {
    await tx.run(
      `INSERT INTO salary_payments (employee_id, amount, period_label, paid_on, method, note, paid_by)
       VALUES (?, ?, ?, COALESCE(?, CURRENT_DATE), ?, ?, ?)`,
      [data.employee_id, amount, data.period_label || null, data.paid_on || null, data.method || 'cash', data.note || null, paidBy]
    );
    ...
    if (amount > 0) {
      ...
      await postJournal(tx, {
        ...
        created_by: paidBy,
        lines: [
          { code: '5030', debit: amount, credit: 0, memo: 'payroll' },
