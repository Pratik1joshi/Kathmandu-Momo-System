...
export function printKot(kot, { size = '80', reprint = false } = {}) {
  ...
  const dest = kot.order_type === 'takeaway' ? 'TAKEAWAY'
    : kot.table_number ? `TABLE ${esc(kot.table_number)}` : 'COUNTER';
...
export function printProforma(proforma, { size = '80', settings = {} } = {}) {
  ...
  const body = `
    ...
    <div class="r-meta">
      ${metaRow(isTakeaway ? 'Type' : 'Table', isTakeaway ? 'Takeaway' : (order.table_number || '—'))}
...
export function printFinalBill(receipt, { size = '80', reprint = false } = {}) {
  ...
  const isTakeaway = !receipt.table_number;
  ...
  const body = `
    ...
    <div class="r-meta">
      ...
      ${metaRow(isTakeaway ? 'Type' : 'Table', isTakeaway ? 'Takeaway' : receipt.table_number)}
