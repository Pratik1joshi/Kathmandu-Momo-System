...
const RESTAURANT = 'Kathmandu Momo';
...
export function printCreditStatement(statement, { size = '80', settings = {} } = {}) {
  const { kind, name, phone, outstanding, lines = [] } = statement;
  ...
  const body = `
    ...
    ${doctypeBlock(kind === 'supplier' ? 'SUPPLIER STATEMENT' : 'CUSTOMER STATEMENT', ['NOT A TAX INVOICE'])}
    <div class="r-meta">
      ${metaFull(kind === 'supplier' ? 'Supplier' : 'Customer', name || '—')}
      ${phone ? metaRow('Phone', phone) : ''}
      <div class="r-meta-row"><span class="l">Printed</span><span class="v">${esc(formatNepalDate())} ${esc(formatNepalClock())}</span></div>
    </div>
    <div class="r-hr"></div>
    ${lines.length ? `<table><thead><tr>
        <th class="c-name" style="width:34%">Date / Note</th>
        <th class="c-price" style="width:22%">Debit</th>
        <th class="c-price" style="width:22%">Credit</th>
        <th class="c-total" style="width:22%">Balance</th>
      </tr></thead><tbody>${rows}</tbody></table>` : `<p class="r-sm" style="text-align:center;padding:4mm 0;">No transactions in this range.</p>`}
    ...
    ${footerBlock({ receipt_footer: 'Please settle outstanding dues at your earliest convenience.' })}`;
  return openReceiptPrint({ title: `${kind === 'supplier' ? 'Supplier' : 'Customer'} statement — ${name || ''}`, size, body, layout: 'bill' });
}
