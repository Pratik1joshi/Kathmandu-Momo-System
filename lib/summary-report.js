...
export async function buildSummaryReport(db, { start, end }) {
  ...
  const [revenue, ledger, purchases, expenses, salaryRows, cashAccount, bankAccount, savings, saleCategories, purchaseCategories, quantities, settings, costMap] = await Promise.all([
    ...
    db.all(`SELECT COALESCE(mc.name,'Uncategorised') AS category,SUM(oi.quantity) AS quantity,SUM(COALESCE(oi.subtotal,oi.quantity*oi.price)) AS amount FROM bills b JOIN orders o ON o.id=b.order_id JOIN order_items oi ON oi.order_id=o.id LEFT JOIN menu_items mi ON mi.id=COALESCE(oi.menu_item_id,oi.item_id) LEFT JOIN menu_categories mc ON mc.id=mi.category_id WHERE b.status IN ('paid','partially_paid') AND date(b.created_at,'+5 hours','+45 minutes') BETWEEN ? AND ? AND COALESCE(oi.status,'') NOT IN ('voided','cancelled') GROUP BY COALESCE(mc.name,'Uncategorised') ORDER BY amount DESC`,[start,end]),
    ...
    Promise.all([
      db.get(`SELECT COALESCE(SUM(oi.quantity),0) AS n FROM bills b JOIN order_items oi ON oi.order_id=b.order_id WHERE b.status IN ('paid','partially_paid') AND date(b.created_at,'+5 hours','+45 minutes') BETWEEN ? AND ? AND COALESCE(oi.status,'') NOT IN ('voided','cancelled')`,[start,end]),
      ...
      db.get(`SELECT COUNT(*) AS n FROM bills WHERE status IN ('paid','partially_paid') AND date(created_at,'+5 hours','+45 minutes') BETWEEN ? AND ?`,[start,end]),
  ...
  const received={cash:round2(revenue.cash+ledger.cash),bank:round2(revenue.bank+ledger.bank)};received.total=round2(received.cash+received.bank);
  const estimatedFoodCost=round2((await db.all(`SELECT COALESCE(oi.menu_item_id,oi.item_id) AS item_id,SUM(oi.quantity) AS qty,SUM(COALESCE(oi.subtotal,oi.quantity*oi.price)) AS amount FROM bills b JOIN order_items oi ON oi.order_id=b.order_id WHERE b.status IN ('paid','partially_paid') AND date(b.created_at,'+5 hours','+45 minutes') BETWEEN ? AND ? AND COALESCE(oi.status,'') NOT IN ('voided','cancelled') GROUP BY COALESCE(oi.menu_item_id,oi.item_id)`,[start,end])).reduce((sum,r)=>sum+(costMap.get(r.item_id)?.cost*num(r,'qty') || num(r,'amount')*.6),0));
  const gross=round2(revenue.total-estimatedFoodCost); const operating=round2(expenses.total+salary.total); const net=round2(gross-operating);
  ...
  return { range:{start,end}, generated_at:new Date().toISOString(), restaurant_name:settings?.restaurant_name||'Restaurant', revenue, ledger, received, purchases, expenses, salary,
    ...
    profit:{revenue:revenue.total,food_cost:estimatedFoodCost,gross,operating,net}, accounts:{cash:cashAccount,bank:bankAccount},
