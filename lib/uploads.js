 ...
 */
async function saveValidatedImage(file, subdir) {
  ...
  const publicMirror = path.join(process.cwd(), 'public', 'uploads', subdir);
  ...
  return `${getImagesPublicPath()}/${subdir}/${filename}`;
}

export async function saveMenuImage(file) {
  return saveValidatedImage(file, 'menu');
}

export async function saveCmsImage(file) {
  return saveValidatedImage(file, 'cms');
}
