...
function channelOf(orderNumber, orderType) {
  if (String(orderNumber || '').startsWith('WEB-')) return 'online';
  if (String(orderType || '') === 'takeaway') return 'takeaway';
  return 'counter';
}
...
export async function listBills(db, {
  ...
  else if (channel === 'takeaway') where.push(`COALESCE(o.order_number,'') NOT LIKE 'WEB-%' AND COALESCE(o.order_type,'')='takeaway'`);
  else if (channel === 'counter') where.push(`COALESCE(o.order_number,'') NOT LIKE 'WEB-%' AND COALESCE(o.order_type,'')<>'takeaway'`);
  if (paymentMethod) {
  ...
  if (canIncludeOpenOrders) {
    ...
    else if (channel === 'takeaway') oWhere.push(`COALESCE(o.order_number,'') NOT LIKE 'WEB-%' AND COALESCE(o.order_type,'')='takeaway'`);
    else if (channel === 'counter') oWhere.push(`COALESCE(o.order_number,'') NOT LIKE 'WEB-%' AND COALESCE(o.order_type,'')<>'takeaway'`);
    if (search) {
  ...
  let enriched = allRows.map((r) => {
    ...
    return {
      ...
      channel: channelOf(r.order_number, r.order_type),
...
export async function getBillDetail(db, id) {
  ...
  return {
    ...
    channel: channelOf(b.order_number, b.order_type),
