...
import { postJournal, postSaleJournal, paymentAccountCode, ensureAccountingSchema } from '@/lib/accounting.js';
...
export async function completeBillPayment(db, {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const journalId = await postJournal(tx, {
      ...
      lines: [
        ...paymentAllocations.map((allocation) => ({ code: paymentAccountCode(allocation.method), debit: allocation.amount, credit: 0, memo: `${allocation.method} payment` })),
