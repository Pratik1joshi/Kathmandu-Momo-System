...
export class OrderRepository {
  ...
  async _insertOrderItem(orderId, item) {
    ...
    const name = item.item_name || item.name || menuItem?.name || 'Item';
    const price = Number(item.price ?? menuItem?.base_price ?? 0);
    const quantity = Number(item.quantity || 1);
    const subtotal = price * quantity;

    // Live schema: price (not unit_price), optional item_id, no variant_id
    ...
    return await this.db.transaction(async () => {
      const result = await this.db.run(`
        INSERT INTO orders (
          ...
          status, notes, customer_name, customer_phone, customer_id, created_at, updated_at
      ...
      if (tableId) {
        // Close any leftover open orders on this table so cashier never sees
        // multiple statuses for the same table.
        await this.db.run(`
          UPDATE orders
      ...
      return { order_id: orderId, order_number: orderNumber, stock };
    });
  }
  ...
  async updateStatus(id, status) {
    ...
    return await this.db.transaction(async () => {
      ...
      // Only free table after payment or cancel — never on serve/dining
      ...
      // Any other open orders on this table are stale
      await this.db.run(`
        UPDATE orders
        SET status = 'cancelled',
            notes = COALESCE(notes, '') || char(10) || 'Auto-cancelled: superseded by current table order',
            updated_at = CURRENT_TIMESTAMP
        WHERE table_id = ?
          AND id != ?
          AND status NOT IN ('completed', 'cancelled')
    ...
    return await this.db.transaction(async () => {
      ...
      await this.db.run(`
        ...
        SET status = 'cancelled',
            ...
            updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
      `, [`\nCancel: ${reason || 'No reason'}`, id]);

      if (wasActive) {
        try {
          const { restoreStockForItems } = await import('../../stock.js');
          const items = await this.getOrderItems(id, { includeVoided: false });
          await restoreStockForItems(this.db, items, { orderId: id, reason: `Order cancelled: ${reason || 'no reason'}` });
        } catch (e) {
  ...
  async createBillAndPayment(billData, tx = null) {
    ...
    const run = async () => {
      const billNumber =
        ...
        `BILL-${Date.now()}-${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
      ...
      // Supplemental orders point their new bill at the immutable original.
      await db.run(
        `UPDATE bills SET parent_bill_id = (
           SELECT reopened_from_bill_id FROM orders WHERE id = ?
         ) WHERE id = ?`,
        [billData.order_id, billId]
      );
  ...
  async getAllPayments(startDate = null, endDate = null) {
    let query = `
      SELECT bp.*, b.bill_number, b.grand_total, o.order_number, o.table_id, t.table_number
