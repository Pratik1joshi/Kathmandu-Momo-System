...
export class OrderRepository {
  ...
  async create(orderData) {
    ...
    const { deductStockForItems, ensureStockSchema } = await import('../../stock.js');
  ...
  async createBillAndPayment(billData, tx = null) {
    ...
    const run = async () => {
      ...
      const billOrder = billData.order_id
        ? await db.get('SELECT order_type, table_id, table_number FROM orders WHERE id = ?', [billData.order_id])
        : null;
