...
const SETTLED_BILL = `LOWER(COALESCE(b.status,'')) = 'paid'`;
...
async function salesAggregate(db, range, businessDayId = null) {
  ...
  return {
    bills: num(bill.bills), grossSales, discounts, refunds,
    netSales: round2(grossSales - discounts - refunds),
...
export async function composeAnalytics(db, range, _filters = {}, options = {}) {
  ...
  const [
    ...
    orderCancelReasons, voidReasons, recipeCoverage,
  ] = await Promise.all([
    safe(db.all(
    ...
    safe(db.all(
      `SELECT CASE
          ...
          WHEN COALESCE(o.order_type,'dine_in')='dine_in' THEN 'dine in'
          WHEN COALESCE(o.order_type,'')='takeaway' THEN 'takeaway'
          WHEN COALESCE(o.order_type,'')='delivery' THEN 'delivery'
          ELSE COALESCE(o.order_type,'counter') END AS channel,
       ...
       GROUP BY CASE
          ...
          WHEN COALESCE(o.order_type,'')='takeaway' THEN 'takeaway'
          WHEN COALESCE(o.order_type,'')='delivery' THEN 'delivery'
          ELSE COALESCE(o.order_type,'counter') END ORDER BY sales DESC`,
      billPeriod.params
    ), []),
    safe(db.all(
  ...
  if (sales.grossSales > 0 && sales.discounts / sales.grossSales > 0.15) attention.push({ severity: 'warning', type: 'control', title: 'Discount rate is above 15%', detail: `${round2((sales.discounts / sales.grossSales) * 100)}% of gross item sales`, href: '/admin/reports?tab=sales' });
  ...
  const categories = categoryRows.map((row) => ({ category: row.category, quantity: num(row.quantity), revenue: round2(row.revenue), orders: num(row.orders), share: sales.grossSales ? round2(num(row.revenue) / sales.grossSales * 100) : 0 }));
  ...
  const secondaryKpis = [
    makeKpi('gross_sales', 'Gross Item Sales', sales.grossSales, 'currency', previousSales.grossSales),
