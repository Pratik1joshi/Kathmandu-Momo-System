...
async function paymentSummary(db, range, businessDayId = null) {
  ...
  const rows = await safe(db.all(
    `SELECT b.id,b.bill_number,b.created_at,b.paid_at,b.grand_total,
            ...
            COUNT(DISTINCT CASE WHEN LOWER(pay.method)='cash' THEN 'cash'
                                WHEN LOWER(pay.method) IN ('credit','due','unpaid') THEN 'due'
                                ELSE 'online' END) AS method_groups
     FROM bills b
  ...
  for (const raw of rows) {
    const row = {
      ...
      amount: round2(raw.grand_total), outstanding: round2(raw.outstanding_amount),
    };
  ...
  return {
    ...
    summary: {
      ...
      total: round2(cash.amount + online.amount + split.amount + due.amount + ledgerCash + ledgerOnline),
      transactions: cash.transactions + online.transactions + split.transactions + due.transactions,
    },
  };
}
...
export async function composeAnalytics(db, range, _filters = {}, options = {}) {
  ...
  const primaryKpis = [
    ...
    { ...makeKpi('operating_profit', 'Operating Profit', pnl.netProfit, 'currency', previousPnl.netProfit), highlight: true },
  ...
  const [transactions, paymentBreakdown, purchasing, orderOperations] = await Promise.all([
  ...
  return {
    ...
    sales: {
      ...
      reportKpis: [
        ...
        makeKpi('net', 'Net Revenue', sales.netSales, 'currency', null, 'positive', 'Final billed value less refunds'),
    ...
    channels,
    payments: { methods: paymentMethods, grossCollected, cashCollected, onlineCollected, refunds: sales.refunds, voidReversals, netCollections, breakdown: paymentBreakdown },
    finance: {
