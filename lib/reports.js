...
async function expenseTotals(db, range, f = {}) {
  ...
  const rows = await db.all(
    `SELECT COALESCE(category, 'other') AS category, COUNT(*) AS count, COALESCE(SUM(amount), 0) AS amount
     FROM expenses
     WHERE ${where.sql} AND (source_type IS NULL OR source_type <> 'purchase')
...
async function overviewTab(db, range, f) {
  ...
  return {
    chips,
    kpis: [
      { key: 'revenue', label: 'Revenue', value: revenue, format: 'currency', change: revChange, highlight: true },
...
async function salesTab(db, range, f) {
  ...
  return {
    chips,
    kpis: [
      { key: 'gross', label: 'Total Item Sales', value: gross, format: 'currency' },
      { key: 'discounts', label: 'Less: Discounts', value: discounts, format: 'currency' },
      { key: 'net_item_sales', label: 'Net Item Sales', value: netItemSales, format: 'currency' },
      { key: 'net', label: 'Net Sales', value: reconciliation.netSales, format: 'currency', highlight: true },
...
async function financeTab(db, range, f) {
  ...
  return {
    ...
    kpis: [
      ...
      { key: 'profit', label: 'Profit', value: profit, format: 'currency', highlight: true },
