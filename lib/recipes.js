...
import { ensureStockMovementsTable } from './stock-movements.js';
import { applyStockChanges, ensureLedgerSchema } from './inventory-ledger.js';
import { upsertLinkedExpense } from './expense-links.js';
  ...
  return db.transaction(async (tx) => {
    ...
    const result = await tx.run(
      `INSERT INTO wastage_log
         (raw_material_id, recipe_id, quantity, unit, reason, logged_by, notes, employee_id, shift, photo_url)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
...
export async function listWastage(
  ...
  if (from) {
    conditions.push('date(w.created_at) >= date(?)');
    params.push(from);
  }
  if (to) {
    conditions.push('date(w.created_at) <= date(?)');
