  ...
  return db.transaction(async (tx) => {
    ...
    if (bill.order_id) {
      const items = await tx.all(
        `SELECT menu_item_id, item_id, quantity FROM order_items
    ...
    await tx.run(
      `INSERT INTO bill_corrections (bill_id, type, amount, reason, restocked, journal_id, created_by)
       VALUES (?, 'void', ?, ?, ?, ?, ?)`,
      [bill.id, round2(bill.grand_total), reason, restocked, journalId, created_by]
    );
...
export async function refundBill(db, { bill_id, bill_number, amount, full = false, method = 'cash', reason, created_by = null }) {
  ...
  return db.transaction(async (tx) => {
    const journalId = await refund(tx, { amount: amt, method, bill_id: bill.id, reason, created_by });
    ...
    await tx.run(
      `INSERT INTO bill_corrections (bill_id, type, amount, reason, journal_id, created_by) VALUES (?, 'refund', ?, ?, ?, ?)`,
      [bill.id, amt, reason, journalId, created_by]
    );
