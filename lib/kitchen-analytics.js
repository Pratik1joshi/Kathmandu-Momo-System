...
export async function kitchenAnalytics(db) {
  ...
  const rows = await db.all(
    `SELECT o.id, o.status, o.created_at, o.prep_started_at, o.ready_at, o.prepared_by, u.full_name AS chef
     FROM orders o LEFT JOIN users u ON o.prepared_by = u.id
     WHERE o.created_at >= ? AND COALESCE(o.status, '') <> 'cancelled'
  ...
  for (const o of rows) {
    ...
    if (o.prepared_by) {
      if (!chefs.has(o.prepared_by)) chefs.set(o.prepared_by, { name: o.chef || 'Unknown', prepared: 0, times: [] });
  ...
  return {
    ...
    chefs: Array.from(chefs.values())
      .map((c) => ({ name: c.name, prepared: c.prepared, avg_prep_minutes: round1(avg(c.times)) }))
