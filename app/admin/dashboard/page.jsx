...
function tableBoardState(table) {
...
const isRunningTable = (table) => tableBoardState(table) === 'running';
const isReservedTable = (table) => tableBoardState(table) === 'reserved';
...
export default function AdminDashboard() {
  ...
  const visibleTableRooms = useMemo(() => tableRooms
    .filter((group) => tableRoomFilter === 'all' || group.room === tableRoomFilter)
    .map((group) => ({
      ...group,
      tables: group.tables.filter((table) => tableStatusFilter === 'all' || tableBoardState(table) === tableStatusFilter),
    }))
    .filter((group) => group.tables.length > 0), [tableRooms, tableRoomFilter, tableStatusFilter]);
  const tableStatusCounts = useMemo(() => tables.reduce((counts, table) => {
    counts[tableBoardState(table)] += 1;
    return counts;
  }, { available: 0, running: 0, reserved: 0 }), [tables]);
  const updatedAgo = (() => {
    void tick;
...
function TableRoomBoard({ tables, rooms, visibleRooms, roomFilter, statusFilter, statusCounts, onRoomFilter, onStatusFilter, onTableClick }) {
  return (
