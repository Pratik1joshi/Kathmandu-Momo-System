...
function Section({title,rows,note}){return <section className="border border-gray-200"><h3 className="border-b border-gray-200 bg-gray-50 px-4 py-3 text-sm font-bold text-gray-900">{title}</h3><div className="divide-y divide-gray-100">{rows.map((r,i)=><div key={r.label} className={`flex justify-between gap-4 px-4 py-2.5 text-sm ${i===rows.length-1?'font-bold':'text-gray-600'}`}><span>{r.label}</span><span className={`tabular-nums ${financialToneClass({...r,label:`${title} ${r.label}`})}`}>{r.sign&&`${r.sign} `}{money(r.value)}</span></div>)}</div>{note&&<p className="border-t border-gray-100 px-4 py-3 text-xs text-gray-500">{note}</p>}</section>}
...
function Category({title,rows}){const tone=title.startsWith('Gross Sale')?'positive':title.startsWith('Purchase')?'negative':undefined;return <section className="border border-gray-200"><h3 className="border-b border-gray-200 bg-gray-50 px-4 py-3 text-sm font-bold">{title}</h3>{rows.length?<div className="divide-y divide-gray-100">{rows.map(r=><div key={r.category} className="flex justify-between px-4 py-3 text-sm"><div><p className="font-medium text-gray-900">{r.category}</p><p className="text-xs text-gray-500">Qty: {Number(r.quantity).toLocaleString()}</p></div><b className={financialToneClass({ label: title, value: r.amount, tone })}>{money(r.amount)}</b></div>)}</div>:<p className="px-4 py-8 text-center text-sm text-gray-500">No {title.toLowerCase()} data</p>}</section>}
