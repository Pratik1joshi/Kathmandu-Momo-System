  ...
  const load = useCallback(async ({ quiet = false } = {}) => {
    ...
    try {
      ...
      if (!dayData.activeSession) {
        const latestDate = dayData.current?.business_date || historyData.rows?.[0]?.business_date || dayData.previous?.business_date;
        const suggestedDate = dayData.storeClosed && dayData.current?.business_date ? dateOnly(dayData.current.business_date) : latestDate ? nextDate(latestDate) : nepalToday();
